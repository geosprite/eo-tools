...
def snap_tool(cls: T) -> T:
    """Register a builtin SNAP Tool class when its module is imported."""
    return register_tool_class(_TOOL_CLASSES, cls)


def discover_builtin_tools() -> list[Tool]:
    """Import every SNAP builtin tool module and instantiate registered tools."""
    global _DISCOVERED
    _DISCOVERED = discover_tool_classes(
        package_name=__package__ or "geosprite.eo.tools.snap",
